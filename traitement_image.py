...
def convolution_matr(matrice, filtre):
    ...
    map(lambda y: map(lambda x: produit_scalaire_mat(x, y, filtre, matrice), range(diff, longueur - diff)),
        range(diff, hauteur - diff))
    return mat2
...
def amincir_peu(image):
    ...
    for x in range(1, longueur - 1):
        for y in range(1, hauteur - 1):
            somme = pixels[x, y - 1] + pixels[x, y + 1] + pixels[x - 1, y] + pixels[x + 1, y] + pixels[x - 1, y - 1] + \
    ...
    for i in l_5:
        for j in l_6:
            ...
            if aucun_bord or px < lower:
                ...
                if canny[j - 1][i - 1][2] or canny[j - 1][i][2] or canny[j - 1][i + 1][2] or canny[j][i + 1][2] or \
                        canny[j + 1][i - 1][2] or canny[j + 1][i][2] or canny[j + 1][i + 1][2]:
                    za.point((i, j), fill="white")
                    canny[j][i][2] = True
                else:
                    za.point((i, j), fill="black")
